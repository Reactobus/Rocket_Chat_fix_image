...
4. Put patched `app.js` next to the Dockerfile, build a derived image (e.g. `:8.7.0-patched`), point `docker-compose.yml` at it, recreate the `rocketchat` service. Use `docker build --no-cache` if the old layer masks a new `app.js`.
